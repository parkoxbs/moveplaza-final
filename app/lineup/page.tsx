...
const getPositionInfo = (topPercent: string, leftPercent: string, matchType: string) => {
  ...
  if (y >= 85) return { label: 'GK', color: 'bg-yellow-400 text-black' };
  let side = 'C';
  if (x < 28) side = 'L'; else if (x > 72) side = 'R';
  if (y >= 68) {
            ...
            <div className="space-y-4">
              ...
              <div className="pt-4 border-t border-white/10 space-y-3 mt-2">
                <div className="grid grid-cols-2 gap-2">
                    ...
                    <button onClick={() => updatePlayer('isMOM', !editingPlayer.isMOM)} className={`py-4 rounded-xl text-sm font-black border transition active:scale-95 ${editingPlayer.isMOM ? 'bg-blue-600 text-white border-blue-400 shadow-md' : 'bg-slate-800 text-slate-400 border-slate-700'}`}>⭐ MOM</button>
                </div>
                <div className="flex items-center bg-slate-800 rounded-xl border border-slate-700 px-5 py-4 justify-between">
                   <span className="text-sm text-slate-300 font-bold">⚽ 득점 기록</span>
                   <div className="flex items-center gap-4">
                       <button onClick={() => updatePlayer('goals', Math.max(0, editingPlayer.goals - 1))} className="w-10 h-10 bg-slate-700 rounded-xl text-white font-black hover:bg-slate-600 transition">-</button>
                       <span className="font-black text-xl w-6 text-center">{editingPlayer.goals}</span>
                       <button onClick={() => updatePlayer('goals', editingPlayer.goals + 1)} className="w-10 h-10 bg-slate-700 rounded-xl text-white font-black hover:bg-slate-600 transition">+</button>
                   </div>
                </div>
              </div>
            </div>
            <button onClick={() => setEditingPlayer(null)} className="mt-6 w-full py-4 bg-white text-slate-900 font-black text-lg rounded-xl hover:bg-slate-200 transition shadow-lg">완료</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-sm sm:p-4 animate-fade-in" onClick={() => setIsLoadModalOpen(false)}>
          <div className="bg-slate-900 border border-white/10 w-full max-w-md h-[80vh] sm:h-auto sm:max-h-[80vh] rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="p-6 border-b border-white/10 flex justify-between items-center">
                <h3 className="font-black text-white text-xl">내 전술 보관함 📂</h3>
                <button onClick={() => setIsLoadModalOpen(false)} className="text-slate-400 hover:text-white p-1"><Icons.X /></button>
            </div>
            <div className="p-4 overflow-y-auto flex-1 custom-scrollbar space-y-3">
                {savedLineups.length === 0 ? (
                    <div className="text-center py-16">
                        <p className="text-5xl mb-4">텅</p>
                    ...
                    savedLineups.map(lineup => (
                        <div key={lineup.id} onClick={() => loadLineup(lineup)} className="bg-slate-800 p-5 rounded-2xl border border-white/5 hover:border-blue-500 hover:bg-slate-800/80 transition cursor-pointer flex justify-between items-center group shadow-sm">
                            ...
                            <button onClick={(e) => deleteLineup(lineup.id, e)} className="p-3 text-slate-500 hover:text-red-500 transition opacity-0 group-hover:opacity-100 bg-slate-950 rounded-full shadow-sm">
                                <Icons.Trash />
                            </button>
                        </div>
                    ))
