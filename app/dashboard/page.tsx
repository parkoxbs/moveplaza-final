...
export default function Dashboard() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const dataUrl = await toPng(reportRef.current, { 
        cacheBust: true, 
        pixelRatio: 2,
        backgroundColor: '#ffffff', // 👈 배경색 흰색 강제 지정 (투명 방지)
        style: { padding: '20px' } 
      })
    ...
    } catch { toast.error("PDF 생성 실패", { id: t }) }
  }
  ...
  const getSeverityColor = (count: number) => {
    if (count >= 5) return "bg-red-500 text-white border-red-600 shadow-md shadow-red-200"
    if (count >= 3) return "bg-orange-500 text-white border-orange-600 shadow-md shadow-orange-200"
    if (count >= 1) return "bg-yellow-400 text-white border-yellow-500 shadow-md shadow-yellow-200"
    return "bg-white text-gray-500 border-gray-200 hover:bg-gray-50"
  }

  const filteredLogs = selectedDate 
    ? logs.filter(l => new Date(l.created_at).toDateString() === selectedDate.toDateString())
    : logs
    ...
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900 pb-24">
      ...
      {shareData && (
        <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center z-[-1] opacity-0 pointer-events-none">
          <div ref={shareCardRef} className="w-[500px] h-[500px] bg-gradient-to-br from-slate-900 to-slate-800 p-8 flex flex-col justify-between text-white relative overflow-hidden font-sans">
            <div className="absolute top-[-50px] right-[-50px] w-[200px] h-[200px] bg-blue-600 rounded-full blur-[90px] opacity-60"></div>
            <div className="absolute bottom-[-50px] left-[-50px] w-[200px] h-[200px] bg-red-600 rounded-full blur-[90px] opacity-50"></div>
            <div className="z-10">
              <div className="flex justify-between items-start mb-4">
                ...
                <p className="text-slate-400 font-bold text-sm">{new Date(shareData.created_at).toLocaleDateString()}</p>
              </div>
              <h1 className="text-4xl font-black leading-tight mb-4 tracking-tight">{shareData.title}</h1>
              <p className="text-slate-300 text-lg font-medium leading-relaxed line-clamp-4">{shareData.content}</p>
            </div>
            <div className="z-10 border-t border-white/10 pt-6 flex justify-between items-end">
              <div>
                <p className="text-slate-500 text-xs font-black tracking-widest mb-1">INTENSITY</p>
                <p className="text-5xl font-black text-white">{shareData.pain_score}<span className="text-xl text-slate-500 ml-1">/ 10</span></p>
              </div>
              <div className="text-right">
                <p className="font-black text-2xl italic tracking-tighter text-white">MOVEPLAZA</p>
                <p className="text-[10px] text-slate-500 font-bold tracking-widest uppercase">Athlete Performance System</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-gray-100 transition-all">
        <div className="max-w-md mx-auto px-5 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => window.location.reload()}>
            <div className="w-8 h-8 bg-blue-900 rounded-lg flex items-center justify-center text-white font-black text-lg shadow-blue-900/20 shadow-lg">M</div>
            <span className="text-xl font-black tracking-tight text-slate-900">MOVEPLAZA</span>
          </div>
          <div className="flex items-center gap-4 text-sm font-bold text-gray-500">
            <Link href="/community" className="hover:text-blue-600 transition">광장</Link>
            <Link href="/my" className="hover:text-blue-600 transition">내 정보</Link>
          </div>
        </div>
      </header>

      {/* 🟢 메인 컨텐츠 (PDF 저장 대상) */}
      <main className="max-w-md mx-auto px-5 pt-8 space-y-8 animate-slide-up" ref={reportRef}>
        ...
        <section>
          ...
          <p className="text-slate-500 font-bold mt-2 text-sm">오늘도 부상 없이 득근해볼까요? 💪</p>
        </section>
        ...
        <section className="bg-white rounded-3xl p-6 shadow-xl shadow-slate-200/60 border border-slate-100 relative overflow-hidden">
          ...
          <div className="flex justify-between items-end mb-6 relative">
            <div>
              <h3 className="text-lg font-black text-slate-900 flex items-center gap-2">부상 히트맵 <span className="text-red-500 animate-pulse"><Icons.AlertCircle /></span></h3>
              <p className="text-xs font-bold text-slate-400 mt-1">최근 통증 부위 (재활 기록만)</p>
            </div>
            <div className="text-right"><span className="block text-3xl font-black text-slate-900">{rehabLogs.length}</span><span className="text-xs font-bold text-slate-400">건의 통증</span></div>
          </div>
            ...
            {bodyParts.map((part) => {
              ...
              return (
                <div key={part} className={`px-3 py-1.5 rounded-xl text-xs font-bold border transition-all duration-300 ${getSeverityColor(count)}`}>
                  {part} {count > 0 && <span className="ml-1 opacity-90 text-[10px]">({count})</span>}
                </div>
              )
            })}
          </div>
        ...
        {/* 📅 캘린더 & 차트 (그래프 애니메이션 꺼서 PDF에 나오게 함) */}
        <section className="bg-white p-6 rounded-3xl shadow-lg border border-slate-100">
           <h3 className="font-extrabold text-slate-900 mb-4">이번 달 활동 흐름</h3>
           <div className="h-40 mb-6">
             <ResponsiveContainer width="100%" height="100%">
               <LineChart data={logs.slice(0, 7).reverse()}>
                 ...
                 {/* 👇 PDF에 그래프 나오게 하려면 isAnimationActive={false} 필수! */}
        ...
        <section>
          <div className="flex justify-between items-center mb-4 px-1">
            <h3 className="text-xl font-black text-slate-900">
              {selectedDate ? `${selectedDate.getMonth()+1}월 ${selectedDate.getDate()}일 기록` : '최근 활동'}
            </h3>
            <div className="flex gap-2">
              <button onClick={handleDownloadPDF} className="text-xs bg-slate-100 text-slate-600 px-2 py-1 rounded-lg font-bold hover:bg-slate-200">📄 리포트 저장</button>
              {selectedDate && <button onClick={() => setSelectedDate(null)} className="text-xs bg-gray-200 px-2 py-1 rounded-lg font-bold">전체보기</button>}
            </div>
          </div>

          <div className="space-y-3">
            {loading ? (
              <div className="text-center py-10 font-bold text-slate-300">로딩 중...</div>
            ) : filteredLogs.length === 0 ? (
              <div className="text-center py-12 bg-white rounded-3xl border-2 border-dashed border-slate-200">
                <p className="text-slate-400 font-bold text-sm">기록이 없습니다.</p>
                <button onClick={() => setIsModalOpen(true)} className="mt-4 text-blue-600 font-black text-sm hover:underline">+ 첫 기록 남기기</button>
              </div>
            ) : (
              filteredLogs.slice(0, 10).map((log) => {
                const isWorkout = log.log_type === 'workout' || (log.pain_score && !log.content.includes('통증'));
                return (
                  <div key={log.id} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between transition hover:shadow-md hover:scale-[1.01] cursor-default group">
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-sm ${isWorkout ? 'bg-blue-50 text-blue-600' : 'bg-red-50 text-red-600'}`}>
                        {isWorkout ? <Icons.Activity /> : <Icons.AlertCircle />}
                      </div>
                      <div>
                        <div className="font-black text-slate-900 text-sm mb-0.5">{log.title}</div>
                        <div className="text-xs font-bold text-slate-500 line-clamp-1">{log.content}</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                       <button onClick={() => handleShare(log)} className="opacity-0 group-hover:opacity-100 p-2 text-slate-400 hover:text-pink-500 hover:bg-pink-50 rounded-full transition" title="인스타 공유"><Icons.Share /></button>
                       <button onClick={() => handleDeleteLog(log.id)} className="opacity-0 group-hover:opacity-100 p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-full transition" title="삭제"><Icons.Trash /></button>
                       <div className="text-right">
                          <div className={`font-black text-lg ${log.pain_score > 7 ? 'text-red-500' : 'text-slate-900'}`}>{log.pain_score}</div>
                          <div className="text-[10px] font-bold text-slate-400">점</div>
                       </div>
                    </div>
                  </div>
                )
      ...
      {/* ➕ 플로팅 버튼 */}
      <div className="fixed bottom-0 left-0 right-0 p-6 pointer-events-none flex justify-end max-w-md mx-auto z-40">
        <button onClick={() => setIsModalOpen(true)} className="pointer-events-auto w-16 h-16 bg-blue-600 rounded-full shadow-xl shadow-blue-600/40 flex items-center justify-center text-white hover:bg-blue-700 transition transform hover:scale-110 active:scale-95"><Icons.Plus /></button>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-0 sm:p-4 animate-fade-in">
          <div className="bg-white w-full max-w-md h-[90vh] sm:h-auto sm:max-h-[85vh] rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-slide-up-modal">
            <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              <h3 className="font-extrabold text-lg text-gray-900">새로운 기록 남기기 ✍️</h3>
              <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-gray-200 rounded-full transition"><Icons.X /></button>
            </div>
            <div className="p-6 overflow-y-auto space-y-6 flex-1">
               <div className="flex bg-gray-100 p-1 rounded-xl">
                 <button onClick={() => setLogType('workout')} className={`flex-1 py-3 rounded-lg font-extrabold text-sm transition ${logType === 'workout' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-400'}`}>💪 운동 완료</button>
                 <button onClick={() => setLogType('rehab')} className={`flex-1 py-3 rounded-lg font-extrabold text-sm transition ${logType === 'rehab' ? 'bg-white text-red-600 shadow-sm' : 'text-gray-400'}`}>🏥 재활/통증</button>
               </div>
